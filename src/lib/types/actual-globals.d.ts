...
declare global {
	interface Window {
		 ...
		 */
		$query:
			| ((<T extends TableName>(query: AQLQuery) => Promise<AQLQueryResult<ActualTable[T]>>) &
					((query: AQLQuery) => Promise<AQLQueryResult<Record<string, unknown>>>))
			| undefined;
		 ...
		 */
		$q: (((table: TableName) => AQLQuery) & ((table: string) => AQLQuery)) | undefined;
		...
		/**
		 * Actual Budget's internal action-creator map, bound to dispatch.
		 * Used for triggering real UI actions (modals, etc.) from outside React.
		 *
		 * @example
		 * window.__actionsForMenu.pushModal({ modal: { name: "add-account", options: {} } })
		 */
		__actionsForMenu: Record<string, (...args: unknown[]) => unknown> | undefined;

...
export type Priority = TemplatePriority;
...
interface PriorityPlannerDeps {
	...
	sheetToMonthKey: (sheet: string) => string | null;
	sheetToMonthLabel: (sheet: string) => string | null;
}
...
function addBreakdownPriorityTier(tierMap: Map<string, PriorityTier>, item: TierRow): void {
	...
	// A category can have multiple templates land in the same priority tier
	// (e.g. two "simple" templates both at priority 3) — merge them into one
	// row per (tier, category) instead of pushing duplicates.
	const existingRow = tier.rows.find((r) => r.catId === item.catId);
	if (existingRow) {
		existingRow.allocatedCents += item.allocatedCents;
		if (item.requestedCents == null) {
			existingRow.requestedCents = null;
		} else if (existingRow.requestedCents != null) {
			existingRow.requestedCents += item.requestedCents;
		}
		existingRow.status = statusFor(existingRow.requestedCents, existingRow.allocatedCents);
	} else {
		tier.rows.push(item);
	}
}
		...
		priorityPromise = (async (): Promise<PriorityCache> => {
			try {
				...
				for (const row of [...regularRows, ...remainderRows]) {
					...
					// A regular default-priority row and a remainder row can both
					// land in the same tier for the same category (REMAINDER_PRIORITY
					// shares the numeric value 0 with unprefixed "#template" lines) —
					// merge them into one row per (tier, category) instead of pushing
					// duplicates, since the UI renders one row per category per tier.
					const existingRow = tier.rows.find((r) => r.catId === row.catId);
					if (existingRow) {
						existingRow.requestedCents += row.requestedCents;
						existingRow.allocatedCents += row.allocatedCents;
						existingRow.gapCents += row.gapCents;
						existingRow.templateCount += row.templateCount || 1;
						existingRow.rawTemplates.push(...(row.rawTemplates || []));
						existingRow.priorities = Array.from(
							new Set([...existingRow.priorities, ...(row.priorities || [row.priority])]),
						).sort(comparePriority);
					} else {
						tier.rows.push(row);
					}

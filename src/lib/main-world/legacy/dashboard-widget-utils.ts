...
const GLOBAL_NAME = "ActualBudgetTweaksDashboardWidgets" as const;
...
export interface DashboardWidgetUtilsGlobal {
	createMarkdownWidgetDefinition(
		args: CreateMarkdownWidgetDefinitionArgs,
	): DashboardWidgetDefinition;
	enhanceAddWidgetMenu(args: EnhanceAddWidgetMenuArgs): void;
	getNativeMenuItemName(button: HTMLElement): string | null;
}

declare global {
	interface Window {
		ActualBudgetTweaksDashboardWidgets?: DashboardWidgetUtilsGlobal;
	}
}
...
function getNativeMenuItemName(button: HTMLElement): string | null {
...
function ensureDashboardWidgetUtils(): DashboardWidgetUtilsGlobal {
	...
	window[GLOBAL_NAME] = utils;
	return utils;
}

export const dashboardWidgetUtils = ensureDashboardWidgetUtils();
